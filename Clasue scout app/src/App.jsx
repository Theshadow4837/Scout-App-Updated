...
// ─── Firebase helpers (mirror the old Supabase API shape) ───────────────────────

// Select documents from a collection with optional equality filters
...
// Insert a document; uses row.id as the document ID if provided
...
// Update documents matching filters with a patch object
...
function idbOpen() {
  return new Promise((res, rej) => {
    const r = indexedDB.open("frc_scout", 2);
    r.onupgradeneeded = e => {
      const db = e.target.result;
      ["users","teams","memberships","forms","submissions","session"].forEach(s => {
...
// ─── Design tokens ──────────────────────────────────────────────────────────────
const C = { bg0:"#07090f", bg1:"#0c1120", bg2:"#111827", border:"#1e2d45", accent:"#38bdf8", orange:"#fb923c", green:"#4ade80", red:"#f87171", muted:"#64748b", text:"#e2e8f0", dim:"#94a3b8" };
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (sess?.user) { setUser(sess.user); if (sess.team) { setTeam(sess.team); setMem(sess.mem); } }
      setBooting(false);
      checkPending();
    })();
  }, []);
  ...
  return (
    <div style={sx.page}>
      ...
      <main style={sx.main}>
        ...
        {user && team && tab === "home"                  && <HomeTab team={team} user={user} role={role}/>}
        {user && team && tab === "forms"                 && <FormsTab team={team} user={user} role={role} online={online} onPending={checkPending}/>}
        {user && team && tab === "data"   && (role === "owner" || role === "admin") && <DataTab team={team}/>}
        {user && team && tab === "manage" && role === "owner"                       && <ManageTab team={team} user={user} onTeamUpdate={t => { setTeam(t); saveSession(user, t, mem); }}/>}
      </main>
    </div>
  );
}
...
function Header({ user, team, role, tab, setTab, onLogout }) {
  ...
  return (
    <header style={sx.hdr}>
      ...
      <nav style={{ display:"flex", gap:4 }}>{tabs.map(t => <button key={t.id} style={sx.nb(tab === t.id)} onClick={() => setTab(t.id)}>{t.l}</button>)}</nav>
  ...
  async function submit() {
    ...
    try {
      if (mode === "signup") {
        ...
        const nu = { id:uid(), username:username.trim(), password_hash:ph, created_at:new Date().toISOString() };
    ...
    } catch (e) { setErr(online ? "Server error — check your Firebase credentials." : "Offline and no cache found."); }
    setBusy(false);
  }
...
function TeamScreen({ user, onJoin, onLogout, online }) {
  ...
  async function createTeam() {
    ...
    try {
      ...
      const pwd  = genPwd();
      const t    = { id:uid(), number:tNum.trim(), name:tName.trim(), password:pwd, owner_id:user.id, created_at:new Date().toISOString() };
    ...
    } catch (e) { setErr("Error creating team — check connection."); }
    setBusy(false);
  }
  ...
  async function joinTeam() {
    ...
    try {
      ...
      onJoin(t, m);
    } catch (e) { setErr("Error joining team."); }
    setBusy(false);
  }
...
// ─── Home Tab ────────────────────────────────────────────────────────────────────
function HomeTab({ team, user, role }) {
  ...
  return (
    <div style={{ maxWidth:800, margin:"0 auto" }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
        ...
        <div style={sx.sc}><div style={{ fontSize:18, fontWeight:700, color:C.orange }}>{role.toUpperCase()}</div><div style={{ fontSize:11, color:C.muted, letterSpacing:2, textTransform:"uppercase" }}>Your Role</div></div>
      </div>
      <div style={sx.card}>
...
function FormsTab({ team, user, role, online, onPending }) {
  ...
  return (
    <div style={{ maxWidth:800, margin:"0 auto" }}>
      ...
      {forms.map(f => (
        <div key={f.id} style={{ ...sx.card, cursor:"pointer" }} onClick={() => { setActive(f); setView("fill"); }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div><div style={{ fontSize:15, fontWeight:700, marginBottom:4 }}>{f.title}</div><div style={{ fontSize:11, color:C.muted }}>{f.questions?.length || 0} questions · by {f.created_by}</div></div>
            <button style={sx.sm()}>FILL →</button>
          </div>
        </div>
      ))}
    </div>
...
function FormBuilder({ team, user, onSave, onCancel }) {
  ...
  async function save() {
    ...
    setBusy(true); setErr("");
    const form = { id:uid(), team_id:team.id, title:title.trim(), questions:qs, created_by:user.username, created_at:new Date().toISOString() };
    try { const saved = await fbInsert("forms", form); await idbPut("forms", saved); onSave(); }
    catch (e) { setErr("Error saving form."); }
    setBusy(false);
  }
...
function FormFiller({ form, user, team, onDone, onCancel, online }) {
  ...
  async function submit() {
    ...
    setBusy(true); setErr("");
    const sub = { id:uid(), form_id:form.id, team_id:team.id, submitted_by:user.username, answers:ans, created_at:new Date().toISOString() };
    try {
      if (online) {
      ...
      setDone(true);
    } catch (e) { setErr("Failed to save submission."); }
    setBusy(false);
  }
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto" }}>
      ...
      <div style={sx.card}>
        ...
        <div style={{ display:"flex", flexWrap:"wrap", gap:12 }}>
          {Object.values(analytics).map((a, i) => (
            <div key={i} style={{ ...sx.sc, minWidth:150, flex:"1 1 150px" }}>
      ...
      <div style={sx.card}>
        ...
        {filtered.map(s => (
          <div key={s.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:`1px solid ${C.border}`, cursor:"pointer" }} onClick={() => setDetail(s)}>
            <div style={{ width:32, height:32, borderRadius:"50%", background:C.bg2, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:700, color:C.accent }}>{s.submitted_by?.[0]?.toUpperCase()}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:13 }}>{s.submitted_by}</div>
              <div style={{ fontSize:11, color:C.muted }}>{new Date(s.created_at).toLocaleString()}</div>
            </div>
            <div style={{ fontSize:11, color:C.muted, maxWidth:280, overflow:"hidden", whiteSpace:"nowrap", textOverflow:"ellipsis" }}>
...
// ─── Manage Tab (Owner) ───────────────────────────────────────────────────────────
function ManageTab({ team, user, onTeamUpdate }) {
  const [members,  setMembers]  = useState([]);
  ...
  async function setRole(m, role) {
    try { await fbUpdate("memberships", { id:m.id }, { role }); const u = { ...m, role }; await idbPut("memberships", u); setMembers(ms => ms.map(x => x.id === m.id ? u : x)); setMsg(`${m.username} is now ${role}.`); setErr(""); }
