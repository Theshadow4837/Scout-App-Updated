...
async function fbInsert(col, row) {
  if (row.id) { await setDoc(doc(db, col, row.id), row); return row; }
...
async function fbDeleteById(col, id) { await deleteDoc(doc(db, col, id)); }
...
export default function App() {
  ...
  // Boot: restore session — membership is stored locally so no rejoin needed
  useEffect(() => {
    (async () => {
      ...
      if (sess?.user) {
        ...
        if (sess.team && sess.mem) {
          // Try to freshen membership from Firebase but fall back to cached
          if (navigator.onLine) {
            try {
              const freshMem = await fbSelect("memberships", { team_id:sess.team.id, user_id:sess.user.id });
              if (freshMem.length) {
                await idbPut("memberships", freshMem[0]);
                setTeam(sess.team); setMem(freshMem[0]);
                await saveSession(sess.user, sess.team, freshMem[0]);
              } else {
                // Membership deleted — user was kicked
                setUser(sess.user); setTeam(null); setMem(null);
                await saveSession(sess.user, null, null);
              }
            } catch {
              setTeam(sess.team); setMem(sess.mem);
            }
          } else {
            setTeam(sess.team); setMem(sess.mem);
          }
  ...
  async function syncPending() {
    ...
    for (const row of rows) {
      try { await fbInsert("submissions", row.data); await idbDel("pending", row.id); } catch {}
    }
    checkPending();
  }
  ...
  async function login(u) {
    ...
    try {
      ...
      if (myMems.length === 1) {
        ...
        if (t) {
          // Verify membership still exists in Firebase (not kicked)
          if (navigator.onLine) {
            try {
              const freshMem = await fbSelect("memberships", { team_id: t.id, user_id: u.id });
              if (freshMem.length) {
                await idbPut("memberships", freshMem[0]);
                setUser(u); setTeam(t); setMem(freshMem[0]);
                await saveSession(u, t, freshMem[0]);
                return;
              }
            } catch {
  ...
  async function leaveTeam() {
    // Find all memberships for this team
    ...
    try {
      await fbDeleteById("memberships", mem.id);
  ...
  return (
    <div style={sx.page}>
      <link href="https://fonts.googleapis.com/css2?family=Roboto+Mono:wght@400;700&display=swap" rel="stylesheet"/>
      {!online && <div style={sx.ob(C.orange)}>⚠ OFFLINE — {pending} queued</div>}
      {online && pending > 0 && <div style={sx.ob(C.green)}>↑ SYNCING {pending}</div>}
      ...
      <main style={sx.main}>
        ...
        {user && team && tab==="announce" && <AnnouncementsTab team={team} user={user} role={role} online={online}/>}
        {user && team && tab==="forms"    && <FormsTab team={team} user={user} role={role} online={online} onPending={checkPending}/>}
        {user && team && tab==="event"    && <EventTab team={team} user={user} role={role} online={online}/>}
        {user && team && tab==="myteam"   && <MyTeamTab team={team} user={user} role={role} online={online}/>}
  ...
  async function submit() {
    ...
    try {
      if (mode==="signup") {
        ...
        const saved = await fbInsert("users",nu); await idbPut("users",saved); onLogin(saved);
      } else {
        let user;
        if (online) {
          ...
          if (!user) { setErr("Offline: no cached credentials."); setBusy(false); return; }
        }
        onLogin(user);
      }
    } catch { setErr(online?"Server error — check Firebase config.":"Offline and no cache."); }
    setBusy(false);
  }
...
function HomeTab({ team, user, role, mem, onDeleteAccount, onDeleteTeam, onLeaveTeam }) {
  ...
  useEffect(()=>{
    (async()=>{
      // Prefer cache for offline support
      const cachedMems = await idbAll("memberships");
      const localMems = cachedMems.filter(m=>m.team_id===team.id);
      if (localMems.length) setMembers(localMems);
      try {
      ...
      const cachedAnns = (await idbAll("announcements")).filter(a=>a.team_id===team.id);
      if (cachedAnns.length) { cachedAnns.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)); setAnnouncements(cachedAnns.slice(0,3)); }
      try {
        const anns=await fbSelect("announcements",{team_id:team.id});
        anns.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
        for(const a of anns)await idbPut("announcements",a);
...
function AnnouncementsTab({ team, user, role, online }) {
  const [announcements,setAnnouncements]=useState([]);
  ...
  useEffect(()=>{
    ...
    (async()=>{
      const cached = await idbAll("announcements");
      const local = cached.filter(a=>a.team_id===team.id);
      local.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
      if (local.length) setAnnouncements(local);
    })();

    if (!online) return;
    const q=query(collection(db,"announcements"),where("team_id","==",team.id));
    const unsub=onSnapshot(q, snap=>{
      const rows=snap.docs.map(d=>({id:d.id,...d.data()}));
      rows.sort((a,b)=>new Date(b.created_at)-new Date(a.created_at));
      setAnnouncements(rows);
      rows.forEach(a=>idbPut("announcements",a));
    });
    return()=>unsub();
  },[team.id, online]);
  ...
  async function deleteAnn(a) {
    ...
    try { await fbDeleteById("announcements",a.id); await idbDel("announcements",a.id); setAnnouncements(as=>as.filter(x=>x.id!==a.id)); } catch { alert("Error deleting."); }
  }
  ...
  return (
    <div>
      ...
      {!online&&<div style={{...sx.card,borderColor:C.orange+"44",color:C.orange,fontSize:12,padding:10}}>⚠ Offline — showing cached announcements</div>}
...
function FormsTab({ team, user, role, online, onPending }) {
  ...
  useEffect(()=>{load();},[]);
  async function load() {
    // Always load cache first for offline
    try { const c=await idbAll("forms"); const local=c.filter(f=>f.team_id===team.id); if(local.length)setForms(local); } catch {}
  ...
  async function deleteForm(f,e) {
    ...
    try { await fbDelete("submissions",{form_id:f.id}); await fbDeleteById("forms",f.id); await idbDel("forms",f.id); setForms(fs=>fs.filter(x=>x.id!==f.id)); }
  ...
  return (
    <div>
      ...
      {!online&&<div style={{...sx.card,borderColor:C.orange+"44",color:C.orange,fontSize:12,padding:10}}>⚠ Offline — showing cached forms</div>}
      {forms.length===0&&(
        <div style={{...sx.card,textAlign:"center",color:C.muted,padding:40}}>
          <div style={{fontSize:40,marginBottom:8}}>📋</div>
...
function FormBuilder({ team, user, editing, onSave, onCancel }) {
  ...
  async function save() {
    ...
    try { await fbInsert("forms",form); await idbPut("forms",form); onSave(); }
...
function FormFiller({ form, user, team, onDone, onCancel, online }) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      try {
        const rows = await fbSelect("scout_events",{team_id:team.id});
        if (rows[0]?.event_teams) setEventTeams(rows[0].event_teams);
      } catch {}
  ...
  async function submit() {
    ...
    try {
      if (online){ try{await fbInsert("submissions",sub);}catch{await idbPut("pending",{data:sub});} }
      else { const idb2=await idbOpen(); idb2.transaction("pending","readwrite").objectStore("pending").add({data:sub}); }
...
function EventTab({ team, user, role, online }) {
  const isPrivileged = role==="owner"||role==="admin";
    ...
    (async()=>{
      ...
      try {
        ...
        if (rows.length) {
          const ev=rows[0];
          setCurrentEvent(ev);
          if (ev.assignment) setAssignment(ev.assignment);
          if (ev.excluded)   setExcluded(ev.excluded);
          if (ev.event_teams) setEventTeams(ev.event_teams);
          await idbPut("scout_events",ev);
        }
        const mrows=await fbSelect("memberships",{team_id:team.id});
        setMembers(mrows);
      } catch {
        const cached = await idbAll("memberships");
        setMembers(cached.filter(m=>m.team_id===team.id));
      }
      // Load pit scouts
      await loadPitScouts();
    })();
  ...
  async function loadPitScouts() {
    try {
      const rows = await fbSelect("pit_scouts",{team_id:team.id});
      setAllPitScouts(rows);
      const mine = rows.filter(r=>r.user_id===user.id).map(r=>r.scouted_team);
      setPitScouted(mine);
      for(const r of rows) await idbPut("pit_scouts",r);
    } catch {
      const cached = (await idbAll("pit_scouts")).filter(r=>r.team_id===team.id);
      setAllPitScouts(cached);
      setPitScouted(cached.filter(r=>r.user_id===user.id).map(r=>r.scouted_team));
    }
  }
  ...
  async function togglePitScout(teamNum) {
    ...
    if (existing) {
      try { await fbDeleteById("pit_scouts",existing.id); await idbDel("pit_scouts",existing.id); }
      catch {}
      ...
      try { await fbInsert("pit_scouts",rec); } catch {}
  ...
  // exportFirst: download CSV before deleting
  // deleteForms: also delete the forms themselves (not just submissions)
  async function doSelectEvent(ev, exportFirst, deleteForms) {
    ...
    try {
      ...
      // Always delete all submissions when switching/ending
      for(const s of allSubs) await fbDeleteById("submissions", s.id);

      // Optionally delete forms too
      if (deleteForms) {
        for(const f of forms) await fbDeleteById("forms", f.id);
      }
      ...
      if (ev) {
        ...
        await fbInsert("scout_events", doc2);
        ...
        // Ending the event — delete the scout_events doc entirely
        const existing = await fbSelect("scout_events",{team_id:team.id});
        for(const e of existing) await fbDeleteById("scout_events", e.id);
  ...
  // ── Delete all event data (keep forms unless asked) ───────────────────────
  async function handleDeleteData(deleteForms) {
    ...
    try {
      ...
      for(const s of allSubs) await fbDeleteById("submissions", s.id);
      if (deleteForms) for(const f of forms) await fbDeleteById("forms", f.id);
      // Reset assignment on the scout_events doc but keep the event
      const existing = await fbSelect("scout_events",{team_id:team.id});
      if (existing.length) {
        const patch = {assignment:null, excluded:[], updated_at:tsNow()};
        await updateDoc(doc(db,"scout_events",existing[0].id), patch);
  ...
  async function generateAssignment() {
    ...
    const existing = await fbSelect("scout_events",{team_id:team.id});
    if (existing.length) await updateDoc(doc(db,"scout_events",existing[0].id),{assignment:newAssignment,excluded,updated_at:tsNow()});
  ...
  async function saveExcluded() {
    ...
    const scouters = members.filter(m=>!excluded.includes(m.user_id));
    const newAssignment = assignTeams(eventTeams, scouters);
    await updateDoc(doc(db,"scout_events",existing[0].id),{assignment:newAssignment,excluded,updated_at:tsNow()});
    setAssignment(newAssignment);
...
// ─── My Team Tab ──────────────────────────────────────────────────────────────
function MyTeamTab({ team, user, role, online }) {
  ...
  useEffect(()=>{
    ...
    if(online) fetchLiveFeed(num, evKey);
    const interval = setInterval(()=>{
      if(document.hidden||!navigator.onLine) return;
      fetchLiveFeed(num, evKey);
    }, 60000);
    const onVisibility = () => {
      if(!document.hidden && navigator.onLine) fetchLiveFeed(num, evKey);
    };
    document.addEventListener("visibilitychange", onVisibility);
    return ()=>{ clearInterval(interval); document.removeEventListener("visibilitychange", onVisibility); };
  },[team.frc_team_number, effectiveEventKey, online]);
  ...
  useEffect(()=>{
    (async()=>{
      ...
      try {
        const rows = await fbSelect("scout_events",{team_id:team.id});
        if(rows[0])setCurrentEventKey(rows[0].event_key);
      } catch {}
    })();
  },[]);
...
function DataTab({ team, user, role }) {
  ...
  async function deleteSub(s,e) {
    ...
    try{await fbDeleteById("submissions",s.id);await idbDel("submissions",s.id);setSubs(ss=>ss.filter(x=>x.id!==s.id));}
...
function ManageTab({ team, user, onTeamUpdate, onRoleChange }) {
  ...
  async function saveName(){
    try{await fbUpdate("teams",{id:team.id},{name:tName});const u={...team,name:tName};await idbPut("teams",u);onTeamUpdate(u);setEditName(false);setMsg("Name updated.");setErr("");}
  ...
  async function togglePitScoutEnabled(){
    ...
    try{
      await updateDoc(doc(db,"teams",team.id),{pit_scout_enabled:val});
      const cached=await idbGet("teams",team.id);
      if(cached)await idbPut("teams",{...cached,pit_scout_enabled:val});
  ...
  async function saveFrcTeamNum(){
    ...
    try{
      await updateDoc(doc(db,"teams",team.id),{frc_team_number:frcTeamNum.trim()});
      const cached=await idbGet("teams",team.id);
      if(cached)await idbPut("teams",{...cached,frc_team_number:frcTeamNum.trim()});
